...
# ---------------------------------------------------------------------------
# User  — student_id is the PRIMARY KEY (no separate integer id)
# ---------------------------------------------------------------------------
class UserModel(Base):
    __tablename__ = "user"

    student_id = Column(String(20), primary_key=True, index=True)  # YYPPNNN, e.g. 2304001
...
class ClubMemberModel(Base):
    ...
    user_id = Column(String(20), ForeignKey("user.student_id", ondelete="CASCADE"), nullable=False)
...
class EventModel(Base):
    ...
    start_time = Column(String(100), nullable=False)     # ISO datetime string e.g. "2026-08-15T18:00"
    end_time = Column(String(100), nullable=True)        # ISO datetime string e.g. "2026-08-15T21:00"
    ...
    # details JSON: {"location": "...", "banner_url": "...", "virtual_link": "...",
    ...
    announcements = relationship("PostModel", back_populates="event", cascade="all, delete-orphan")
...
class PostModel(Base):
    ...
    title = Column(String(255), nullable=False)
    description = Column(Text, nullable=False)
    # post_type: 'post' (feed), 'project' (showcase), 'announcement' (club), 'announcement_event'
    post_type = Column(String(50), nullable=False, default="post")
    # status: 'draft', 'published', 'archived'
    status = Column(String(50), nullable=False, default="published")
    user_id = Column(String(20), ForeignKey("user.student_id", ondelete="CASCADE"), nullable=True)
    club_id = Column(Integer, ForeignKey("club.id", ondelete="CASCADE"), nullable=True)
    ...
    event = relationship("EventModel", back_populates="announcements")
