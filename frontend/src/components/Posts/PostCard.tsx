...
import { MessageSquare, Play, FileText, Download, ExternalLink, Hash, ThumbsUp, ThumbsDown, Star, Send, CornerDownRight } from 'lucide-react';
// Import theme hook context to fix markdown styles
import type { PostData, PostComment, ReactionType, CommentReactionType } from './PostInterfaces';
...
export const PostCard: React.FC<{ postData: PostData }> = ({ postData }) => {
  ...
  const { theme } = useTheme(); // Pull active theme state
  ...
  return (
    // REFACTORED: Layout uses adaptive background variables instead of hardcoded dark hex codes
    <div className="bg-card border border-customBorder rounded-xl overflow-hidden shadow-xl mb-6 w-full transition-all duration-200">
      ...
      <div className="p-5 pb-3 flex items-center justify-between border-b border-customBorder/30">
        <div className="flex items-center space-x-3">
          <div className="w-10 h-10 rounded-xl bg-primary border border-customBorder flex items-center justify-center text-sm transition-colors">{postData.author.avatar}</div>
          <div>
            <h3 className="text-sm font-bold text-mainText transition-colors">{postData.author.name}</h3>
            <p className="text-xs text-subText mt-0.5 transition-colors">{postData.author.roleTitle} • {postData.createdAt}</p>
          </div>
        </div>
        <div className="text-[10px] font-bold px-2 py-0.5 rounded bg-primary text-subText border border-customBorder uppercase tracking-widest">{postData.author.association}</div>
      </div>
      ...
      <div className="p-5 space-y-4">
        ...
        {/* Pass active theme state variable into editor rendering windows */}
      ...
      <div className="px-5 py-3 bg-footer border-t border-customBorder flex flex-wrap items-center justify-between gap-3 transition-colors">
        <div className="flex items-center gap-2">
          <button onClick={() => handleReactionClick('LIKE')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all cursor-pointer ${currentUserReaction === 'LIKE' ? 'bg-emerald-500/10 border-emerald-500 text-emerald-400' : 'bg-primary border-customBorder text-subText hover:text-mainText'}`}><ThumbsUp className="w-3.5 h-3.5" /> {totalLikes}</button>
          <button onClick={() => handleReactionClick('DISLIKE')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all cursor-pointer ${currentUserReaction === 'DISLIKE' ? 'bg-rose-500/10 border-rose-500 text-rose-400' : 'bg-primary border-customBorder text-subText hover:text-mainText'}`}><ThumbsDown className="w-3.5 h-3.5" /> {totalDislikes}</button>
          <button onClick={() => handleReactionClick('STAR')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border transition-all cursor-pointer ${currentUserReaction === 'STAR' ? 'bg-amber-500/10 border-amber-500 text-amber-400' : 'bg-primary border-customBorder text-subText hover:text-mainText'}`}><Star className={`w-3.5 h-3.5 ${currentUserReaction === 'STAR' ? 'fill-current' : ''}`} /> {totalStars}</button>
        </div>

        <button onClick={() => setIsCommentBoxOpen(!isCommentBoxOpen)} className={`text-xs font-bold flex items-center gap-1.5 transition-colors cursor-pointer ${isCommentBoxOpen ? 'text-accent' : 'text-subText hover:text-accent'}`}>
                  ...
                  <div key={root.id} className="space-y-3 relative">
                    ...
                    <div className="bg-card p-4 rounded-xl border border-customBorder shadow-md transition-colors">
                      <div className="flex gap-3 items-start">
                        <div className="text-sm shrink-0 w-8 h-8 rounded-lg bg-primary border border-customBorder flex items-center justify-center">{root.authorAvatar}</div>
                        <div className="space-y-1.5 flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-extrabold text-mainText">{root.authorName}</span>
                            <span className="text-[9px] text-subText font-mono bg-primary px-1.5 py-0.5 rounded border border-customBorder">{root.createdAt}</span>
                          </div>
                      ...
                      return (
                        <div key={reply.id} className="flex gap-2.5 pl-7 items-start">
                          ...
                          <div className="flex-1 bg-card/60 border border-customBorder p-3.5 rounded-xl flex gap-3 items-start shadow-sm transition-colors">
                            <div className="text-xs shrink-0 w-7 h-7 rounded-lg bg-primary border border-customBorder flex items-center justify-center">{reply.authorAvatar}</div>
                            <div className="space-y-1.5 flex-1 min-w-0">
                              <div className="flex items-center justify-between"><span className="text-xs font-bold text-mainText">{reply.authorName}</span><span className="text-[9px] text-subText font-mono">{reply.createdAt}</span></div>
